...
## v3.5.3 — console spacing part 2 + hardened deployment (July 25, 2026)

Scope: exactly the two requested workstreams plus their direct downstream requirements. Public site rendering, copy, colors, type, and behavior untouched.

**A. Console spacing, part 2 (verified by before/after screenshots at 1440 and 390)**

1. **Section rhythm on every pane** — v3.5.2 gave only Settings a 48px break before each section H2; the other eight panes never got it, so "Verified numbers", "Home sections", "Pricing", "FAQ", "Questions", "Shipped", "Photos", "Per-page titles" and the rest sat compressed against the block above. One `body.dash .dash-pane > h2` rule now applies the same 48px (40px mobile) break everywhere, with first-section and intro-hint exceptions so panes do not open with a hole.
2. **Pane-level action rows** — the SEO generate row, Analytics connect row, and the galleries auto-categorize chip now carry the same 12px/24px margins Settings action rows already had, instead of sitting flush against neighbors.
3. **Operator notes** — same generalization for pane-level `.operator-note` blocks.
4. **Footer pane fine print** — the platform list now breathes 16px before its fine print, matching every `.item-list`.
5. **Mobile top bar (≤879.98px)** — wrapped header rows tighten to the 8px grid step and the Preview / Save / Publish buttons stretch to share their rows edge-to-edge instead of wrapping into a ragged third line. Desktop untouched.

Same 4px grid, same `body.dash` scope, margins/paddings/gaps only, appended to the existing spacing system in `css/bndr.css`.

**B. Hardened deployment: .env credentials, email reset, public-repo safety**

1. **`server.mjs` (new, zero dependencies)** — serves the identical site and verifies the owner passphrase on the server from the `OWNER_PASS_KDF` environment variable: rate-limited (8 attempts / 15 min / IP), constant-time compare, byte-identical PBKDF2 to the browser implementation. Adds security headers (nosniff, frame, referrer, permissions, HSTS) and refuses to serve `.env`, `server.mjs`, `package.json`, and `tests/`.
2. **Non-destructive email reset** — "Forgot the passphrase? Email me a one-time code" sends an 8-digit single-use code (10-minute expiry, 3 requests/hour) to `OWNER_EMAIL` via Resend (`RESEND_API_KEY`) or, keyless, via FormSubmit — the relay the estimate form already uses. Entering the code opens the console with all pages, drafts, signing keys, and issued access links intact. Only a salted hash exists anywhere, so a passphrase can never be emailed — only recovered forward.
3. **No credentials in the repo or in `content.js`** — in server mode, Settings → Owner passphrase prints the new `OWNER_PASS_KDF` value for Railway Variables instead of writing any record into the draft/`content.js`. `.env` is git-ignored; `.env.example` ships empty values only.
4. **Enforcement** — `tests/source-audit.mjs` now fails if a real `.env` exists, `.env` is not ignored, `server.mjs` hardcodes a credential or drops rate limiting/constant-time compares, `content.js` ships a legacy unsalted hash or a sub-600k-round record, or any scanned file matches secret patterns (API keys, private keys, plaintext password literals). 400 checks pass.
5. **Graceful fallback** — without the variables (any static host), the console boots the original static gate and typed-RESET recovery unchanged.

Downstream: cache-busted all thirteen pages to `?v=3.5.3`; `package.json` (start script for Railway), `.gitignore`, `.env.example`, README "Hardened deployment" section. Verified live: correct/incorrect passphrase, rate limiting kicking in at the 9th attempt, bad reset code rejected, `.env`/`server.mjs` returning 404, headers present; `node --check` on all scripts and the full audit suite pass.
