...
(function () {
  ...
  /* ── optional server-verified sign-in (v3.5.3 hardening) ──
     When the site runs behind server.mjs with OWNER_PASS_KDF set, the gate
     verifies on the server: no credential record ships to the browser and
     attempts are rate limited. Without the server or the variable, every
     flow below falls back to the original static gate unchanged. */
  var SERVER_AUTH = { enabled: false, reset: false, resetSession: false };
  ...
  function detectServerAuth() {
    if (!window.fetch || location.protocol === "file:") return Promise.resolve({ enabled: false, reset: false, resetSession: false });
    return fetch("/api/console/config", { credentials: "same-origin" })
      .then(function (response) { if (!response.ok) throw new Error("static"); return response.json(); })
      .then(function (config) { return { enabled: !!(config && config.serverAuth), reset: !!(config && config.resetEmail), resetSession: false }; })
      .catch(function () { return { enabled: false, reset: false, resetSession: false }; });
  }
  /* Non-destructive owner recovery: a one-time code goes to the owner email;
     pages, drafts, signing keys, and issued access links are all kept. */
  function serverReset() {
    var err = $("#gate-err");
    if (!SERVER_AUTH.reset) { err.textContent = "Reset email is not configured — set OWNER_EMAIL on the server and redeploy."; return; }
    err.textContent = "Sending a one-time code to the owner email…";
    api("/api/console/reset").then(function (result) {
      if (!result.ok) throw new Error(result.error || "The reset email could not be sent.");
      var code = prompt("A one-time code was emailed to the owner address.\n\nNothing on the site was changed — pages, drafts, and access links are all intact. Enter the code to open the console, then set a new passphrase in Settings right away.");
      if (code === null) { err.textContent = ""; return; }
      return api("/api/console/reset/confirm", { code: String(code).trim() }).then(function (confirmed) {
        if (!confirmed.ok) throw new Error(confirmed.error || "That code could not be verified.");
        SERVER_AUTH.resetSession = true;
        try { sessionStorage.setItem(AUTH_KEY, "1"); } catch (e) {}
        openConsole("owner");
        toast("Reset code accepted — set a new passphrase in Settings now");
      });
    }).catch(function (problem) { err.textContent = problem.message || "The reset email could not be sent."; });
  }
  ...
  function paneSettings() {
    ...
    p.appendChild(el("p", "dash-hint", SERVER_AUTH.enabled
      ? "This deployment verifies sign-in on the server from the OWNER_PASS_KDF environment variable — no credential ships in content.js. Setting a new passphrase prints the new variable value: paste it into your host (Railway → service → Variables) and redeploy."
      : "Changes the passphrase in the draft. Publish + deploy for it to take effect on the live console."));
    ...
    passBtn.addEventListener("click", function () {
      ...
      if (SERVER_AUTH.enabled) {
        /* After an emailed one-time code, possession of the owner inbox already
           re-authenticated this session, so the current passphrase is waived once. */
        var reauth = SERVER_AUTH.resetSession
          ? Promise.resolve({ ok: true })
          : api("/api/console/verify", { passphrase: currentPassInput.value });
        reauth.then(function (result) {
          if (!result.ok) throw new Error(result.status === 429 ? (result.error || "Too many attempts — wait a few minutes") : "Current passphrase is not correct");
          return newPassphraseRecord(v);
        }).then(function (record) {
          SERVER_AUTH.resetSession = false;
          envOut.value = JSON.stringify(record);
          envField.style.display = "";
          envOut.focus();
          currentPassInput.value = "";
          passInput.value = "";
          toast("New OWNER_PASS_KDF generated — update the variable and redeploy");
    ...
    p.appendChild(act1);
    var envField = el("div", "field");
    envField.style.display = "none";
    envField.innerHTML = "<label>New OWNER_PASS_KDF value</label>";
    var envOut = el("textarea");
    envOut.readOnly = true;
    envOut.rows = 3;
    envOut.addEventListener("focus", function () { envOut.select(); });
    envField.appendChild(envOut);
    envField.appendChild(el("p", "dash-hint", "Paste this whole line as the OWNER_PASS_KDF variable on your host, then redeploy — the old passphrase keeps working until the redeploy finishes. This is a salted hash, not your passphrase."));
    p.appendChild(envField);
  ...
  function boot() {
    ...
    var firstSetup = !SERVER_AUTH.enabled && !(ownerRecord.passKdf && ownerRecord.passKdf.hash) && !ownerRecord.passHash;
    if (firstSetup) {
    ...
    if (resetBtn) {
      ...
      if (SERVER_AUTH.enabled) resetBtn.textContent = "Forgot the passphrase? Email me a one-time code";
      resetBtn.addEventListener("click", function () {
        if (SERVER_AUTH.enabled) { serverReset(); return; }
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (SERVER_AUTH.enabled) {
        api("/api/console/verify", { passphrase: pass }).then(function (result) {
          if (result.ok) {
            try { sessionStorage.setItem(AUTH_KEY, "1"); } catch (e2) {}
            try { sessionStorage.removeItem(OPERATOR_AUTH_KEY); } catch (e3) {}
            openConsole("owner");
            ...
            var err = $("#gate-err");
            err.textContent = result.status === 429 ? (result.error || "Too many attempts — wait a few minutes.") : "Not it. Try again.";
            form.classList.remove("shake");
            void form.offsetWidth;
            form.classList.add("shake");
          }
        }).catch(function () { $("#gate-err").textContent = "The sign-in service did not respond — try again."; });
        return;
      }
  ...
  detectServerAuth().then(function (info) {
    SERVER_AUTH = info;
    boot();
  });
})();
